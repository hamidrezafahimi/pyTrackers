...
from lib.utils.plotting import get_thresh_precision_pair,get_thresh_success_pair
from lib.utils.vision import calAUC

# TODO: Clean this, like you did for the rest
...
def get_preds_by_name(preds_dict,key):
    valid_keys=['gts','kcf_gray_preds','kcf_hog_preds','dcf_gray_preds',
                ...
                'strcf_preds', 'csrdcf_preds', 'tracker_dimp_preds', 'tracker_kys_preds',
                'tracker_tomp_preds','tracker_prdimp_preds', 'tracker_mixformer_preds']
...
def draw_plot(results_json_path,datalist,dataset_name):
    ...
    for data_name in results.keys():
        ...
        dimp50_preds = get_preds_by_name(data_all, 'tracker_dimp_preds')
        # prdimp50_preds = get_preds_by_name(data_all, 'tracker_prdimp_preds')
    ...
    plt.legend()
    plt.grid()
    plt.savefig(pth + "/results/plots/" + dataset_name + '_precision.pdf', format="pdf")
    plt.clf()
    # plt.show()
    ...
    plt.legend()
    plt.grid()
    plt.savefig(pth + "/results/plots/" + dataset_name + '_success.pdf', format="pdf")
    plt.clf()
    print(dataset_name,':',num_videos)


if __name__=='__main__':
    result_json_path= pth + '/results/all_results.json'
